...
/**
 * Builtin function that reads a long from the {@link SLContext#getInput() standard input}.
 */
@NodeInfo(shortName = "lueppaNumero")
public abstract class SLReadNumberBuiltin extends SLBuiltinNode {

    @Specialization
    public Long readln() {
        Long result = doRead(getContext().getInput());
        if (result == null) {
            /*
    ...
    @TruffleBoundary
    private Long doRead(BufferedReader in) {
        try {
